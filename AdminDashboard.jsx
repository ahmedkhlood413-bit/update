...
export default function Dashboard() {
  ...
  const [activeSidebarPage, setActiveSidebarPage] = useState('dashboard'); // 'dashboard' أو 'orders'
  ...
  const handleDownloadCSV = () => {
    ...
    const headers = Object.keys(processedData[0]).filter(k => k !== 'horseImage' && k !== 'initial' && k !== 'vetStatus').join(',');
    const rows = processedData.map(row => Object.entries(row).filter(([k]) => k !== 'horseImage' && k !== 'initial' && k !== 'vetStatus').map(([_, v]) => `"${v}"`).join(',')).join('\n');
    const blob = new Blob(['\uFEFF' + headers + '\n' + rows], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a'); link.href = url; link.setAttribute('download', `${activeSidebarPage === 'orders' ? 'Orders' : activeTab}.csv`);
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };

  const getOrderStatusStyle = (status) => {
    if(status === 'مكتمل') return 'bg-green-100 text-green-600';
    if(status === 'قيد الانتظار' || status === 'قيد المراجعة') return 'bg-yellow-100 text-yellow-600';
    if(status === 'ملغى') return 'bg-red-100 text-red-600';
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-2 gap-y-6 gap-x-8 text-sm">
                  {Object.entries(modalConfig.data).map(([key, value]) => {
                    if (key === 'id' || key === 'horseImage' || key === 'initial' || key === 'vetStatus') return null;
                    return (
      ...
      <aside className="w-64 bg-[#141b2d] text-gray-300 flex flex-col justify-between hidden md:flex z-10">
        <div>
          ...
          <nav className="space-y-2 px-4 text-sm font-medium">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition text-gray-400"><i className="fas fa-horse w-5 text-center"></i> الخيول</button>
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition text-gray-400"><i className="fas fa-notes-medical w-5 text-center"></i> السجلات الطبية</button>
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition text-gray-400"><i className="fas fa-cog w-5 text-center"></i> الإعدادات</button>
          </nav>
        </div>
        <div className="p-4 bg-[#1a233a] flex items-center justify-between m-4 rounded-xl cursor-pointer">
           <div><h4 className="text-white text-sm font-semibold">أحمد محمد</h4><p className="text-xs text-gray-400">مدير النظام</p></div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white px-8 py-4 flex items-center justify-between border-b border-gray-100 z-10">
          <div className="flex items-center gap-8">
            <h2 className="text-xl font-bold text-gray-800">{activeSidebarPage === 'dashboard' ? 'إدارة النظام والطلبات' : 'إدارة الطلبات الشاملة'}</h2>
        ...
        <div className="flex-1 overflow-auto p-8 bg-gray-50/50">
          ...
          {/* ======================= صفحة الطلبات (الجديدة والتفاعلية) ======================= */}
          {activeSidebarPage === 'orders' && (
             <div className="animate-fade-in-up">
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  {/* 🟢 شريط الفلترة وتحميل الإكسيل الخاص بالطلبات */}
                  <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {processedData.map((order) => (
                        <tr key={order.id} className="hover:bg-gray-50 transition">
                          ...
                          {/* 🟢 أزرار الأكشن لجدول الطلبات تعمل الآن مثل لوحة التحكم */}
